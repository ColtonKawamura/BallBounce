...
vecMassHat     = [20, 40];               % ball-to-chain mass ratios
        ...
        for idxV = 1:length(vecVImpactHat)
            ...
            for idxDamp = 1:length(vecDampHat)
                ...
                if scalDampMax == scalDampMin
                    % all dampings equal: just use a single fixed marker size
                    vecMarkerSize = 8 * ones(size(vecDampHat));  % or any value you like
                else
                    vecMarkerSize = 4 + 8 * (vecDampHat - scalDampMin) / (scalDampMax - scalDampMin);  % 4–12
                end
                semilogx(NArr, e, ...
